...
(async () => {
  console.log('🚀 RALPH MODE - FIXED VERSION');
  ...
  const pollForDnsPage = async () => {
    ...
    try {
      ...
      // Use page.waitFor() instead of waitForTimeout
    ...
    if (!dnsFound && attempts < maxAttempts) {
      ...
      // Success! Close browser after 5 seconds
      setTimeout(() => {
        browser.close().then(() => {
          console.log('');
  ...
  setTimeout(() => {
    if (!dnsFound) {
      browser.close().then(() => {
        ...
        console.log('');
        console.log('⚠️ Manual nameserver change required:');
        console.log('  1. Go to Cloudflare DNS settings');
        console.log('  2. Change nameservers to:');
